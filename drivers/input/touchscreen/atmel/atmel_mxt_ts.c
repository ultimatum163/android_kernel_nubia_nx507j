...
#define ZTEMT_C_AREA_WIDTH	0 //lee for 508
...
static int zte_ignore_zone(int point_id, int x, int y) //point_id
{
    ...
    {
        if(x<ZTEMT_C_AREA_WIDTH || x>(1080-ZTEMT_C_AREA_WIDTH)){//also in c zone
            return IGNORE_TURN_C;
        }else{//c-a
            slot_ignore[i].slot_zoneid = ZONE_A;
            return IGNORE_TURN_C_TO_A;
        }
    }else if((slot_ignore[i].slot_zoneid) == ZONE_A){
    ...
    }else{//(slot_ignore[i].slot_zoneid) == 11    //new point//ZONE_DEFAULT
            if(x<ZTEMT_C_AREA_WIDTH || x>(1080-ZTEMT_C_AREA_WIDTH)){// in c zone
                slot_ignore[i].slot_zoneid = ZONE_C;
                return IGNORE_TURN_C;
            }else{// in a zone
...
static void mxt_reinit_t15_key_order(struct mxt_data *data)
{
	...
	if (ret == 0) {
		if (suffix[0] != '\0')
			ver = strncmp(suffix, ".03", strlen(suffix));
	}
...
/*ZTEMT start add for panel without rim*////////////////////////
	...
	/* For multi touch */
	error = input_mt_init_slots(input_rim_dev, data->num_touchids/*, INPUT_MT_DIRECT*/);
	if (error) {
...
/*** ZTEMT start ,20150123***/
static DEVICE_ATTR(ic_detect, S_IRUSR /*| S_IWGRP | S_IWOTH*/, mxt_ic_detect_show,NULL);
static DEVICE_ATTR(manual_cali, S_IRUSR /*| S_IWGRP | S_IWOTH*/, mxt_manual_cali_show,NULL);
static DEVICE_ATTR(easy_wakeup_gesture, S_IWUSR /*| S_IRUSR | S_IWGRP | S_IWOTH*/, NULL, mxt_easy_wakeup_gesture_store);//mxt_plugin_wakeup_gesture_show
...
#if 0
  	/* reset gpio */
	pdata->gpio_reset = of_get_named_gpio_flags(dev->of_node,"atmel,reset-gpio", 0, NULL);  
#else
...
static struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
	...
	*/
	...
	if (prop) {
		...
		keymap[T15_T97_KEY][0] = KEY_BACK;//KEY_MENU;
		keymap[T15_T97_KEY][1] = KEY_MENU;//KEY_BACK;
		keymap[T15_T97_KEY][2] = KEY_HOMEPAGE;

		keymap[T81_KEY][0] = /*KEY_F10*/KEY_WAKEUP;
		keymap[T93_KEY][0] = /*KEY_F10*/KEY_WAKEUP;
#else
...
#if !defined(CONFIG_MXT_EXTERNAL_TRIGGER_IRQ_WORKQUEUE)
        dev_err(&client->dev, "1 zte lee atmel data->pdata->irqflags= %ld \n",(data->pdata->irqflags));
        data->pdata->irqflags = IRQF_TRIGGER_LOW;
